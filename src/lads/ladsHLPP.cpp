...
void LADS::modifyAlphaAndResetNetworkOverdensed(Graph * g, double alpha, Network & network,
        ...
        double volBR) {
    ui eCnt = 0;
    std::vector<ui> ct(g->n+2);
    for(ui u : R) {
        if(Aset.find(u) == Aset.end())
            network.reSetEdge(s, u+1, ct[s]++, ct[u+1]++, g->degree(u));
        else
            network.reSetEdge(s, u+1, ct[s]++, ct[u+1]++, 1e15);
    }

    for(ui u = 0; u < g->n; u++) {
        for(ui i = g->pIdx[u]; i < g->pIdx[u+1]; i++) {
            ui v = g->pEdge[i];
            network.reSetEdge(u+1, v+1, ct[u+1]++, ct[v+1]++, 1);
        }
    }
// printf("%lu\n", overdensed.size());
// for(ui u : Aset) printf("%u %d ", u, (int)overdensed[u]); printf("\n");fflush(stdout);
    for(ui u = 0; u < g->n; u++) {
        if(overdensed[u])
            network.reSetEdge(u+1, t, ct[u+1]++, ct[t]++, volBR+1);
        else  {
            network.reSetEdge(u+1, t, ct[u+1]++, ct[t]++, alpha);
        }
    }  
}