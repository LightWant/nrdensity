...
printf("stCoreU:%u\n", stu);
    ui edges = 0;
    for(ui u = stu; u < g->n; u++) {
        edges += g->pIdx[u+1] - g->pIdx2[u];
    }
    return 1.0 * edges / (g->n - stu);
}

double ccpath::realDensityInMaximumClique(ui & maxCliqueU) {
    ui dc = 1;
    for(ui u = g->n-2; u > 0; u--) {
        bool ok = true;
        for(ui v = u + 1; v < g->n; v++) {
            if(!g->connect(u, v)) {
                ok = false; break;
            }
        }
    ...
    ui tmpu = 0;
    for(ui u = g->n-dc-1; u > 0; u--) {
        if(g->pIdx[u+1]-g->pIdx2[u] <= dc) continue;
        ui dc2 = 1;
        for(ui j = g->pIdx[u+1]-1; j > g->pIdx2[u]; j--) {
            ui v = g->pEdge[j-1];
            bool ok = true;
    ...
    maxCliqueU = tmpu;
    printf("maxCliqueU:%u\n", tmpu);
printf("DegeneraceClique:%u\n", dc);

    return 1.0 * cb(dc, k) / dc;
}
...
double ccpath::appDensity(bool biased) {
    ...
    ui maxCliqueU = 0;
    // double rkcore = realDensityInMaximumClique(maxCliqueU);
std::cout << "edgeDensity:" << rcore << std::endl;
// std::cout << "cDensityInDegeneraceClique:" << rkcore << std::endl;
    // rcore = std::max(rcore, rkcore);
    ui stUOfMaxCore = 0;
    for(ui u = 0; u < g->n; u++) {
    ...
    //     if(cb(d, k) >= rkcore) {
    //     // if(g->pIdx[u+1] - g->pIdx2[u] >= g->coreNumber) {
    //         stUOfMaxCore = std::max(stUOfMaxCore, u);
    //         break;
    //     }
    // }
...
// stUOfMaxCore = std::min(stUOfMaxCore, maxCliqueU);
