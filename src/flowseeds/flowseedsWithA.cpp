//implementation that considers strong/anchored nodes

#include "flowseedsWithA.h"
#include <set>
...
void flowseeds::globalPushRelabel(
    ...
    for(ui u : R) {
        if(Aset.find(u) == Aset.end())
            network.addEdge(s, u+1, alpha*g->degree(u));
        else
            network.addEdge(s, u+1, g->m);//INF
    }
    for(ui u = 0; u < g->n; u++) {
        if(inR[u]) continue;
...
double flowseeds::cutval(Graph * g, const std::vector<ui> & S,
    ...
    double pAnotInS = 0;
    for(ui u : R) {
        if(Aset.find(u) != Aset.end() && Sset.find(u) == Sset.end()) {
            pAnotInS += (double)g->m*10 * g->degree(u);
        }
    }
    double relcond = cutS / (volRS - (volS-volRS) - pAnotInS);
#ifdef DEBUG
printf("cts %f\nvolRs %f\nvolS %f\nedges %f\nPea %f\nrealcond %f xx\n\n", 
    cutS, volRS, volS,  edgeS*2, pAnotInS, relcond);
#endif
    return relcond;
}
...
ui L = 0;
        ...
        for(ui ul : C_local) if(inR[Local2Global[ul]]) {
            if(Aset.find(Local2Global[ul]) == Aset.end())
                network.addEdge(s, ul+1, alpha*g->degree(Local2Global[ul]));
            else
                network.addEdge(s, ul+1, alpha*g->m*10*g->degree(Local2Global[ul]));
#ifdef DEBUG
M++;
#endif
        }
...
printf("\n");
        ...
        for(ui i = 0; i < I_local.size(); i++) {
            ...
            ui u = Local2Global[ul];
            if(isInI_global[u] == true) {
                if(newI_local_size == i) newI_local_size++;
                else I_local[newI_local_size++] = ul;
            }
        }
