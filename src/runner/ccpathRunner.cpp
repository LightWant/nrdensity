...
int main(int argc, char * argv[]) {
    ...
    Graph g;
    if(ac.exist("-f_txt")) {
        std::cout << ac["-f_txt"] << std::endl;
        g.readFromText(ac["-f_txt"], noVUM);
    }
    else if(ac.exist("-f_txtD")) {
        std::cout << ac["-f_txtD"] << std::endl;
        g.readFromTextDoubleEdges(ac["-f_txtD"]);
    }
    else {
        std::cout << ac["-f"] << std::endl;
        g.readFromBin(ac["-f"]);
    }
    ...
    double rho = cp.appDensity(ac.exist("-biased"));
